 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class local_sernobre_mcp_legacy_sections_external extends external_api {
    ...
    public static function create_sections($courseid, $position, $number) {
        ...
        if (! ($course = $DB->get_record('course', array('id' => $params['courseid'])))) {
            throw new moodle_exception('invalidcourseid', 'local_sernobre_mcp', '', $courseid);
        }
        ...
        if (!$courseformat->uses_sections()) {
            throw new moodle_exception('courseformatwithoutsections', 'local_sernobre_mcp', '', $courseformat);
        }
        ...
        if ($desirednumsections > $maxsections) {
            throw new moodle_exception('toomanysections', 'local_sernobre_mcp', '',
    ...
    public static function delete_sections($courseid, $sectionnumbers = [], $sectionids = []) {
        ...
        if (!empty($sectionids)) {
            foreach ($coursesections as $section) {
                if (in_array((int)$section->id, array_map('intval', $sectionids), true)) {
                    $secnums[] = (int)$section->section;
                }
            }
    ...
    public static function move_section($courseid, $sectionnumber, $position) {
        ...
        if ($sectionnumber < 0 or $sectionnumber > $lastsectionnumber) {
            throw new moodle_exception('invalidsectionnumber', 'local_sernobre_mcp', '',
                array('sectionnumber' => $sectionnumber, 'lastsectionnumber' => $lastsectionnumber));
        }

        // Move section.
        if (!move_section_to($course, $sectionnumber, $position)) {
            throw new moodle_exception('movesectionerror', 'local_sernobre_mcp');
        }
    ...
    public static function update_sections($courseid, $sections) {
        ...
        foreach ($sections as $sectiondata) {
            ...
            try {
                ...
                if ($found == 0) {
                    throw new moodle_exception('sectionnotfound', 'local_sernobre_mcp', '', $sectiondata['section']);
                }
                ...
                $warning = array();
                $warning['sectionnumber'] = $sectiondata['section'];
    ...
    public static function create_section_parameters() {
        return new external_function_parameters([
            'courseid' => new external_value(PARAM_INT, 'id of course'),
            'name' => new external_value(PARAM_RAW, 'section name', VALUE_DEFAULT, ''),
            ...
            'position' => new external_value(PARAM_INT, 'position; 0 appends', VALUE_DEFAULT, 0),
            'visible' => new external_value(PARAM_INT, 'visibility', VALUE_DEFAULT, 1),
        ]);
    }

    public static function create_section($courseid, $name = '', $summary = '', $summaryformat = 1,
            $position = 0, $visible = 1) {
        global $DB;
        $params = self::validate_parameters(self::create_section_parameters(), compact(
            'courseid', 'name', 'summary', 'summaryformat', 'position', 'visible'));
        $created = self::create_sections($params['courseid'], $params['position'], 1);
        $item = reset($created);
        $section = $DB->get_record('course_sections', ['id' => $item['sectionid']], '*', MUST_EXIST);
        $data = ['type' => 'id', 'section' => (int)$section->id, 'name' => $params['name'],
            'summary' => $params['summary'], 'summaryformat' => $params['summaryformat'],
            'visible' => $params['visible']];
        $updated = self::update_sections($params['courseid'], [$data]);
        if (!empty($updated['warnings'])) {
            throw new moodle_exception('error', 'local_sernobre_mcp', '', $updated['warnings']);
        }
        $section = $DB->get_record('course_sections', ['id' => $section->id], '*', MUST_EXIST);
        return ['action' => 'created', 'sectionid' => (int)$section->id, 'sectionnum' => (int)$section->section];
    }

    public static function create_section_returns() {
        return new external_single_structure([
            'action' => new external_value(PARAM_ALPHA, 'created'),
            'sectionid' => new external_value(PARAM_INT, 'section id'),
    ...
    public static function update_section($courseid, $sectionid, $name = null, $summary = null,
            $summaryformat = 1, $visible = null) {
        $params = self::validate_parameters(self::update_section_parameters(), compact(
            'courseid', 'sectionid', 'name', 'summary', 'summaryformat', 'visible'));
        $data = ['type' => 'id', 'section' => $params['sectionid']];
        foreach (['name', 'summary', 'summaryformat', 'visible'] as $key) {
            if ($params[$key] !== null) {
                $data[$key] = $params[$key];
            }
        }
        if (count($data) === 2) {
            throw new moodle_exception('error', 'local_sernobre_mcp', '', 'No section fields supplied');
        }
        $updated = self::update_sections($params['courseid'], [$data]);
        if (!empty($updated['warnings'])) {
            throw new moodle_exception('error', 'local_sernobre_mcp', '', $updated['warnings']);
        ...
        return ['action' => 'updated', 'sectionid' => (int)$section->id, 'sectionnum' => (int)$section->section];
    }
